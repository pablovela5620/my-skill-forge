...
TODO (unproven path for large recordings): registering the `.rrd` on an OSS catalog server may lift the size gate — `rerun server --cors-allow-origin '<viewer-origin>'`, then `rr.catalog.CatalogClient(uri).create_dataset(...).register([file_uri])` and point the viewer at `dataset.segment_url(seg)` (see `tools/apps/dataset_collector.py` in rerun-io/so100-hackathon). Status from a 751 MB test on 0.34.0: registration is instant (lazy indexing) and the **native** viewer browses the catalog fine, but the **WASM** viewer fails connection verification with "missing grpc-status trailer" in every topology — even same-machine plain-http against the binary's own served web viewer, with curl-verified correct grpc-web framing — so the web-embed half is blocked upstream, not by infrastructure. Two further hurdles once that's fixed: tailscale serve (tcp+TLS and https modes) truncates gRPC streams, so tailnet transport needs a gRPC-aware proxy; and the OSS server is in-memory (1.6 GB RSS after serving the 751 MB recording), so it has its own RAM ceiling.
